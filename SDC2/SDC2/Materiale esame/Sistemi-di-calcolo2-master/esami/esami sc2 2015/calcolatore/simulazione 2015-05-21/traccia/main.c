...
void listen_on_port(unsigned short port_number_no) {
    ...
    while (1) {
        ...
        /** INSERIRE CODICE PER LANCIARE UN THREAD PER "chat_session()"
         *
         * Obiettivo: allocare dinamicamente una struttura "session_thread_args_t"
         *            e completarne i campi (si veda" common.h"); creare quindi un
         *            thread che esegua la routine "chat_session()" passando il
         *            puntatore della struttura appena allocata come argomento
         *
         * Indicazioni:
         * - i valori per i campi "socket" ed "address" della struct per gli
         *   argomenti sono disponibili dopo che la "accept()" ha avuto successo
         * - non verranno fatte operazioni di join sul thread creato
         *
         **/
...
int main(int argc, char* argv[]) {
    ...
    /** INSERIRE CODICE PER LANCIARE IL THREAD PER "broadcast_routine()"
     *
     * Obiettivo: creare un thread che esegua la routine "broadcast_routine()",
     *            che non necessita di alcun argomento da passare
     *
     * Attenzione: non verranno fatte operazioni di join sul thread creato
     *
     **/
