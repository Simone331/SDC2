...
void initMemory() {
     ...
     **/
    if(shm_unlink(SH_MEM_NAME)!=0) handle_error("shm_unlink");
    fd_shm= shm_open(SH_MEM_NAME, O_CREAT | O_EXCL | O_RDWR, 0666);
    if (fd_shm<1) handle_error("error shm_open ");
    if(ftruncate(fd_shm, sizeof(struct shared_memory))==-1) handle_error("ftruncate");
    
    myshm_ptr= mmap(0, sizeof(struct shared_memory), PROT_READ | PROT_WRITE,  MAP_SHARED,fd_shm, 0);
    if (myshm_ptr==MAP_FAILED) handle_error("mmap");
...
void closeMemory() {
     ...
     **/
      if(munmap(myshm_ptr, sizeof(struct shared_memory))!=0) handle_error("munmap");
