...
int write_to_pipe(int fd, const void *data, size_t data_len) {
    ...
    while (written_data<data_len){
    	ret= write(fd, data + written_data, data_len - written_data);
    	if(ret==-1){
    		if (errno==EINTR) continue;
    		handle_error("cannot write");
		}
		written_data+=ret;
	}
	return written_data;
...
int read_from_pipe(int fd, void *data, size_t data_len) {
    ...
    int ret;
    int r_data=0;
    while (r_data<data_len){
    	ret=read(fd,data+r_data, data_len-r_data);
    	if (ret==0) handle_error("waaaaaaaaaaaaaa");
    	if (ret==-1){
    		if (errno=EINTR) continue;
...
int main(int argc, char* argv[]) {
    ...
    ret=pipe(pipefd);
    if(ret==-1) handle_error("errore creazione pipe");
